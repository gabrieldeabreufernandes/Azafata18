...
public class CanComService extends Service {
    ...
    public void onEventBackgroundThread(final ComSendEvent event) {
        ...
        //Log.d(TAG, "onEventBackgroundThread: MSG TO SEND \n" + canMSG.toString());

        if(canMSG != null) {
            ////IMPORTANT LOG////
            //Log.i(TAG, "AN MSG FROM TABLET = " + canMSG.toString());
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                if(canMSG != null) {
                    Log.i(TAG, "onEventBackgroundThread: CAN MSG FROM TABLET = " + canMSG.getData().toString());
                    deviceCom.sendCommand(canMSG);
                }
            }
        }, 300); //ms message interval*/
    }
}
